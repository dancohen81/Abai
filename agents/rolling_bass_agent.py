...
# Placeholder for Rolling Bass Agent's system prompt (to be loaded from file)
SYSTEM_PROMPT = ""
try:
    with open("ableton_ai_controller/prompts/rolling_bass_system_prompt.txt", "r") as f:
        SYSTEM_PROMPT = f.read()
except FileNotFoundError:
    SYSTEM_PROMPT = "Error: Rolling Bass system prompt file not found."
...
async def process_request(user_input: str, conversation_history: list, current_mode: str, initial_session_info: dict, context_knowledge: str = None) -> str:
    ...
    """
    print(f"RollingBassSpecialistAgent: Processing request for '{user_input}' using LLM (Mode: {current_mode})...")

    # Construct the system message for the Rolling Bass LLM, including the current mode and initial session info
    ...
    try:
        ...
        if isinstance(parsed_response, dict) and parsed_response.get("clarification_needed"):
            return f"Rolling Bass Specialist: Clarification needed: {parsed_response.get('question')}"
        ...
        for command in commands_to_execute:
            ...
            if cmd_type in ableton_api_functions:
                ...
                results.append(f"RollingBassSpecialistAgent: Unknown command type '{cmd_type}' from LLM.")
        
        return "Rolling Bass Specialist: " + "\n".join(results)

    except json.JSONDecodeError:
        return f"Rolling Bass Specialist: LLM response was not valid JSON. Raw response: {llm_response_str}"
    except Exception as e:
        return f"Rolling Bass Specialist: Error during LLM processing: {str(e)}"
