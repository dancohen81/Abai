...
Specialized agent for handling arrangement tasks.
"""
...
from ableton_api_wrapper import (
    create_midi_track, set_track_name, create_clip, set_clip_name,
    fire_clip, stop_clip, start_playback, stop_playback,
    get_session_info, get_track_info, create_audio_track,
...
# Placeholder for Arrangement Agent's system prompt (to be loaded from file)
SYSTEM_PROMPT = ""
try:
    with open("ableton_ai_controller/prompts/arrangement_system_prompt.txt", "r") as f:
        SYSTEM_PROMPT = f.read()
except FileNotFoundError:
    SYSTEM_PROMPT = "Error: Arrangement system prompt file not found."
...
async def process_request(user_input: str, conversation_history: list, current_mode: str, initial_session_info: dict, context_knowledge: str = None) -> str:
    """
    Processes a request related to arrangement using an LLM.
    """
    print(f"ArrangementSpecialistAgent: Processing request for '{user_input}' using LLM (Mode: {current_mode})...")

    # Construct the system message for the Arrangement LLM, including the current mode and initial session info
    ...
    try:
        ...
        if isinstance(parsed_response, dict) and parsed_response.get("clarification_needed"):
            return f"Arrangement Specialist: Clarification needed: {parsed_response.get('question')}"
        ...
        for command in commands_to_execute:
            ...
            if cmd_type in ableton_api_functions:
                ...
                results.append(f"ArrangementSpecialistAgent: Unknown command type '{cmd_type}' from LLM.")
        
        return "Arrangement Specialist: " + "\n".join(results)

    except json.JSONDecodeError:
        return f"Arrangement Specialist: LLM response was not valid JSON. Raw response: {llm_response_str}"
    except Exception as e:
        return f"Arrangement Specialist: Error during LLM processing: {str(e)}"
